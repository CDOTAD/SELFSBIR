...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, low_dim=128, in_channel=3, width=1, output_layer=7, stage='pretrain'):
        self.inplanes = 64
    ...
    def forward(self, x, use_ckpt=False):
        if use_ckpt:
            ...
            x = cp.checkpoint(self.bn1, x)
            x = cp.checkpoint(self.relu, x)
            x = cp.checkpoint(self.maxpool, x)
            ...
            if self.output_layer == 5:
                return x
            x = cp.checkpoint(self.avgpool, x)
            x = x.view(x.size(0), -1)
            if self.output_layer == 6 and self.stage == 'finetune':
                cat = cp.checkpoint(self.fc, x)
                return cat, x
            x = cp.checkpoint(self.mlp, x)
            x = cp.checkpoint(self.fc, x)
            x = cp.checkpoint(F.normalize, x, 2, 1)
            return x
        else:
            if self.output_layer <= 0:
                return x
