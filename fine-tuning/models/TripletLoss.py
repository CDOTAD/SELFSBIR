...
class TripletLoss(nn.Module):
    ...
    def forward(self, sketch, photo):
        ...
        pos_distance = torch.sqrt(torch.sum(pos_distance, dim=1) + self.eps)
        ...
        negative_distance = torch.sqrt(torch.sum(negative_distance, dim=2) + self.eps)

        triplet_loss = pos_distance - negative_distance
